...
export function TodoList() {
  ...
  const { data: testData, isLoading: _testLoading, error: _testError } = trpc.todo.test.useQuery();
  ...
  const toggleCompletedMutation = trpc.todo.toggleCompleted.useMutation({
    ...
    onError: (error) => {
      showNotification('error', `Failed to update todo status: ${error.message}`);
    },
  });
    ...
    <div className="space-y-6">
      {notification.type && (
        ...
        >
          ...
          {notification.type === 'error' && <AlertTriangle className="w-5 h-5 text-red-600 mr-2" />}
          {notification.type === 'info' && <Info className="w-5 h-5 text-blue-600 mr-2" />}
      ...
      {testData && (
        <Win98Window title="System Status" className="mb-4">
          <div className="p-4 bg-[#c0c0c0] text-sm">
            ...
            <p className="mb-1">Todo Model Available: {testData.hasTodoModel ? 'Yes ✓' : 'No ✗'}</p>
        ...
        <Win98Window title="ERROR" className="mb-4">
          <div className="p-4 bg-red-50">
            <div className="flex items-start">
              ...
              <p className="text-red-700">Error loading todos: {todosError.message}</p>
            </div>
