...
import { NFTMintHeader } from './NFTDetailHeader';
...
export function NFTDetailContent({ address, id, role = 'user' }: NFTDetailContentProps) {
  // State for dialogs
  ...
  const { data: connectedAddress } = useAddress();
  ...
  const isOwner = useMemo(() => {
    if (!connectedAddress || !nftOwner) return false;
    return connectedAddress.toLowerCase() === nftOwner.toLowerCase();
  }, [connectedAddress, nftOwner]);
  ...
  return (
    <>
      ...
      <div className="bg-[#0A0A0A] border border-[#1f1f1f] rounded-lg p-6">
        ...
        {/* Dialogs - Only show Transfer dialog if connected wallet is the owner */}
        {isOwner && (
          <TransferDialog
