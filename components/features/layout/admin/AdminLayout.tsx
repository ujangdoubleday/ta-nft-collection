...
export interface AdminCollectionContextType {
  isOwner: boolean;
  collectionAddress: string | null;
  isLoading: boolean;
}

export const AdminCollectionContext = createContext<AdminCollectionContextType>({
  isOwner: false,
...
export const useAdminCollectionContext = () => useContext(AdminCollectionContext);
...
export function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const { links } = useAdminNavigation({ isOwner });
    ...
    if (pathname.startsWith('/admin/collections')) {
      if (collectionAddress) {
        breadcrumbs.push({
          ...
          icon: null,
          isCurrentPage: pathname === `/admin/collections/${collectionAddress}`,
        });
  ...
  const contextValue: AdminCollectionContextType = {
    isOwner,
    collectionAddress,
    isLoading,
  };
  ...
  return (
    <AdminCollectionContext.Provider value={contextValue}>
      <div className="bg-[#0A0A0A] navigation-container">
        ...
        <div className="md:hidden relative" style={{ zIndex: 10000 }}>
          ...
          {/* Breadcrumbs below navbar on mobile */}
          <Container className="pb-2 px-2 border-b border-[#1f1f1f]">
            <Breadcrumb>
        ...
        {/* Mobile Menu - Content-only overlay */}
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
                ...
                {links.map((link, index) => {
                  ...
                  return (
                    ...
                    >
                      <Link
                        href={link.href}
                        className={`text-2xl font-medium flex items-center justify-center gap-3 ${
      ...
      <div className="content-wrapper">
        <Container className="py-6 px-3 sm:px-4 md:px-6 mx-auto max-w-[84rem] flex-grow">
