...
export function CollectionCard({ id, name, description, count, thumbnail }: CollectionCardProps) {
  return (
    <Win98Window key={id} title={name} className="overflow-hidden">
      <div className="p-3 flex gap-3">
        <div className="win98-shadow-inset h-20 w-20 bg-white flex-shrink-0 flex items-center justify-center">
          <Image src={thumbnail} alt={name} width={96} height={96} className="max-h-16 max-w-16" />
        </div>
